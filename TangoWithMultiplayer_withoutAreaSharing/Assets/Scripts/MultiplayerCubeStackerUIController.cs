...
public class MultiplayerCubeStackerUIController : Photon.PunBehaviour, ITangoAreaDescriptionEvent, ITangoLifecycle
{
    ...
    public void AddCube()
    {
        ...
        Plane plane;
        if (m_pointCloud.FindPlane(cam, centerPoint, out hitPointOnRealWorld, out plane))
        {
            int xIndex = (int)hitPointOnRealWorld.x + (WORLD_SIZE / 2);
            int yIndex = (int)hitPointOnRealWorld.y + (WORLD_SIZE / 2);
            int zIndex = (int)hitPointOnRealWorld.z + (WORLD_SIZE / 2);
            ...
            Vector3 p = (hitPointOnRealWorld * m_cubeSize) - new Vector3(0.0f, m_cubeSize / 2.0f, 0.0f);
